...
\newcommand{\dueDate}{Wednesday, July 6 at 1:00 PM}
...
\begin{parts}
    \part[1] \textbf{Select One:} Did you use \LaTeX{} for the entire written portion of this homework?
...
$$\boldsymbol{\beta}=
    ...
    \begin{subparts}
        ...
        % YOUR ANSWER
        % Change \choice to \CorrectChoice for the appropriate selection/selections 
        \choice Remove the last column of $\boldsymbol{\beta}$
        \choice Remove the first row of $\zv$
        \choice Append a value of 1 to be the first entry of $\zv$.
        \choice Append an additional column of 1's to be the first column of $\boldsymbol{\beta}$ 
        \choice Append a row of 1's to be the first row of $\boldsymbol{\beta}$ 
        \choice Take the transpose of $\boldsymbol{\beta}$
        \end{checkboxes}
        }
...
The following questions should be completed after you work through the programming portion of this assignment. \textbf{For any plotting questions, you must use curves/line graphs, title your graphs, label your axes, and provide units (if applicable) and a legend in order to receive full credit.}
...
\subpart[2] In the handout folder, we provide \texttt{metrics\_sgd\_small.txt}, a text file with the validation cross-entropy loss values for SGD performed without AdaGrad using 100 epochs, 50 hidden units, random init, and 0.01 learning rate. In the \textbf{same figure}, plot them against your validation results for SGD \textbf{with} AdaGrad using the same set of parameters and the small dataset. The x-axis should be epoch number, the y-axis should be average cross-entropy loss, and there should be one curve for SGD without AdaGrad validation loss and one curve for SGD with AdaGrad validation loss.
...
tiny_train_out.txt tiny_validation_out.txt \
    ...
    \Procedure{SGD}{Training data $\Dc_{train}$, \ntset\thinspace data $\Dc_t$} 
      ...
      \For{$e \in \{1, 2, \ldots, E\}$} \Comment{For each epoch}
        ...
        \For{$(\xv, \yv) \in \Dc$} \Comment{For each training example (No shuffling)}
          ...
          \State $\alphav \gets \alphav - \eta \gv'_{\alphav}$
          \State $\betav \gets \betav - \eta \gv'_{\betav}$
